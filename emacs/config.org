...
Company provides good autocomplete options.
...
Integrating =jupyter= into =org-mode=. It needs the path to the =jupyter=
executable before =org-babel= is loaded.

#+BEGIN_SRC emacs-lisp
  (add-to-list 'exec-path "~/miniconda3/envs/quant/bin/")
  (use-package jupyter
    :after (conda))
#+END_SRC
...
Use =lstlisting= for source environments.
...
#+BEGIN_SRC emacs-lisp
  ...
  \\lstset{frame=single,
           ...
           breaklines=true,
           postbreak=\\mbox{\\textcolor{Red}{$↪$}\\enspace},
           keepspaces=true,
